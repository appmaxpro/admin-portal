...
class MapEvaluationContext extends EvaluationContext {
  ...
  MapEvaluationContext({this.data=const {}, this.parent});
  ...
  @override
  bool contains(String name) {
    return data.containsKey(name) || (parent!= null ? parent.contains(name) : false);
  }
}
...
class CallMethodExpression extends QuickExpression {
  ...
  Object evaluate(EvaluationContext context) {
    ...
    Object r = Function.apply(func, paramvalues);
    return r;
  }
...
class CodEqExpression extends QuickExpression {
  QuickExpression test;
  QuickExpression left;
  QuickExpression right;

  CodEqExpression(this.test, this.left, this.right);

  @override
  Object evaluate(EvaluationContext context) {
    var testValue = test.evaluate(context);

    if (testValue == false || testValue == null ||
        (testValue is String && testValue.isEmpty) ||
        (testValue is num && testValue.toInt() == 0))
  ...
  @override
  String toCodeString() {
    return '${test.toCodeString()}?${left.toCodeString()}${right.toCodeString()}';
  }
}

class CodExpression extends QuickExpression {
  QuickExpression test;
  QuickExpression next;
  QuickExpression elseExpr;

  CodExpression(this.test, this.next, this.elseExpr);
  ...
  Object evaluate(EvaluationContext context) {
    ...
    return elseExpr != null ? elseExpr.evaluate(context) : false;
  }

  @override
  String toCodeString() {
    return '#if ${test.toCodeString()}\n${next.toCodeString()}\n'
           '${elseExpr?.toCodeString()??""}';
  }
}

class DelegateExpression extends QuickExpression {
  QuickExpression item;
  DelegateExpression(this.item);
  ...
  @override
  String toCodeString() {
    return item.toCodeString();
  }
}

class LoopExpression extends QuickExpression {
  QuickExpression list;
  Token body;
  String asName;
  Token elseToken;
  LoopExpression(this.list, this.body, this.asName, this.elseToken);

  @override
  Object evaluate(EvaluationContext context) {
    final listVals = list.evaluate(context);

    if (listVals is Iterable && listVals.isNotEmpty){
        int index = 0;
        for (final item in listVals){
          final exprBody = expressionParser.parseExpression([body]);

          final childContext = MapEvaluationContext(parent: context, data: {
            'index': index,
            asName: item
          });
          exprBody.evaluate(childContext);
          index++;
        }
    } else if (elseToken != null) {
      final elseBody = expressionParser.parseExpression([elseToken]);
