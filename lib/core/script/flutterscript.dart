...
""");
  ...
  FlutterScript(this.lisp) {
    ...
    lisp.def("flutterscript/function", 1, (List arguments) {
      dynamic lambda = arguments.first;
    ...
    lisp.def("List", -1, (List args) {
      ...
      for (dynamic cell = args.first; cell != null; cell = cell.cdr) {
        result.add(cell.car);
      }
      return result;
    });
  ...
  Future defn(String functionName, DartFn function) async {
    fns[functionName] = function;
    return await eval("""(defmacro $functionName (&rest args)
              `(dart/funcall \"$functionName\" (dart/parameters ,@args)))
        """);
  }

  void defineType(Type type, Map<String, DartMethod> methods) {
    if (methodsOf[type] == null) {
      methodsOf[type] = methods;
    }
  }

  void addClass(String name, DartClass type) {
    defineClass(name, type.constructor, type.methods);
  }

  Object defineClass(String name, DartFn constructor, Map<String, DartMethod> methods) {
...
class DartParameters extends Macro {
  ...
  @override Cell expandWith(interpreter, Cell arg) {
    ...
    List<dynamic> positional = <dynamic>[];
    List<MapEntry<Sym, dynamic>> named = [];
    Sym key = null;

    foldl(arg, arg, (dynamic body, dynamic item) {
    ...
    Cell pcur = pbody;
    positional.forEach((dynamic item) {
